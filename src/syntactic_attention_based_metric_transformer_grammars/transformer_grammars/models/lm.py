...
from transformer_grammars.models import core, embedding_layer
...
class GeneralizedTXLLanguageModel(hk.Module):
    """Generalized Transformer-XL-based language model with attention weight output."""
        ...
        **kwargs,
        ...
        Returns:
            A tuple containing:
                ...
                - layer outputs of shape [batch_size, seq_len, num_layers, d_model]
                - attention weights dictionary with structure:
                  {
                    'restricted': {layer_idx: weights},
                    'unrestricted': {layer_idx: weights}
                  }
                  where weights have shape [batch_size, num_heads, seq_len, seq_len+memory_len]
        """
        bs, seqlen = seq.shape
        ...
        core_output, new_memory, layers_outputs, attention_weights = self._core(
        ...
        return output, layers_outputs, attention_weights
