...
def switch(funcs, indicator, *args, **kwargs):
    ...
    masked_outs = [_apply_mask_to_pytree(x, mask) for (x, mask) in zip(outs, masks)]
    return jax.tree_map(sum, *masked_outs)


def switch_with_aux(funcs, aux_func, indicator, *args, **kwargs):
    """Applies one of the functions with auxiliary outputs, depending on the indicator value.

    Args:
        funcs: List of base functions to switch between
        aux_func: Function that wraps the base functions and returns (output, aux)
        indicator: Integer tensor indicating which function to use
        *args: Positional arguments to pass to the functions
        **kwargs: Keyword arguments to pass to the functions

    Returns:
        A tuple of (main_output, aux_output) where:
            main_output: The primary output from the selected function
            aux_output: The auxiliary output from the selected function
    """
    num_classes = len(funcs)
    assert indicator is not None or len(funcs) == 1
    ...
    if len(funcs) == 1:
        f = funcs[0]
        return aux_func(f, *args, **kwargs)

    # Apply the aux_func to each base function
    outs_and_auxs = [aux_func(f, *args, **kwargs) for f in funcs]
    outs = [out for out, _ in outs_and_auxs]
    auxs = [aux for _, aux in outs_and_auxs]

    # Create masks for each function
    masks = [jnp.equal(indicator, i) for i in range(num_classes)]

    # Mask both main outputs and auxiliary outputs
    masked_outs = [_apply_mask_to_pytree(x, mask) for (x, mask) in zip(outs, masks)]
    masked_auxs = [_apply_mask_to_pytree(x, mask) for (x, mask) in zip(auxs, masks)]

    # Sum up the masked outputs and auxiliary outputs
    main_output = jax.tree_map(sum, *masked_outs)
    aux_output = jax.tree_map(sum, *masked_auxs)
...
class MultiHeadAttention(hk.Module):
    ...
    def __call__(
        ...
        """Computes the attention values and weights.

        We use the following shape conventions: `B` for batch size, `T` for
        chunk size, `M` for memory length and `D` for the embedding dimension.
        ...
        Returns:
            A tuple containing:
                ...
                - Original attention weights of shape [B, H, T, T]
                - Value heads of shape [B, T + M, H, V]
        """
        ...
        # Store both original and normalized attention weights
        ...
        return outputs, attention_weights, value_heads
...
# HACK: In our case, we dont' use this function,
# so this is not supported to output the value heads.
        ...
        *,
        ...
        Returns:
            A tuple containing:
                ...
                - dictionary containing original and normalized attention weights
                  for restricted and unrestricted heads
        """
        if attn_indicator is None:
            assert num_attns == 1
        batch_size, seq_len, _ = inputs.shape
        attn_vecs = []
        attention_weights = {"restricted": {}, "unrestricted": {}}

        if num_restricted_heads > 0:

            def restricted_attn_fn(attn, inputs, **kwargs):
                attn_vec, orig_weights, norm_weights = attn(inputs=inputs, **kwargs)
                return attn_vec, {"original": orig_weights, "normalized": norm_weights}

            restricted_outputs = switch_with_aux(
                restricted_attns,
                restricted_attn_fn,
                attn_indicator,
                inputs=inputs,
            ...
            restricted_attn_vec, restricted_weights = restricted_outputs
            attention_weights["restricted"] = restricted_weights
            attn_vecs.append(restricted_attn_vec)

        if num_unrestricted_heads > 0:
            unrestricted_attn_vec, orig_weights, norm_weights = unrestricted_attn(
                inputs=inputs,
            ...
            attention_weights["unrestricted"] = {
                "original": orig_weights,
...
def _make_block(
    *,
    ...
    """Generalized Transformer-XL block that returns both outputs and attention weights.

    Args:
        layer: Layer number
        mha_kwargs: Multi-head attention keyword arguments
        ffw_kwargs: Feed-forward network keyword arguments
        dropout_rate: Dropout rate
        num_heads: Total number of attention heads
        num_unrestricted_heads: Number of unrestricted attention heads (unused in this implementation)
        embedding_size: Size of embeddings
        num_attns: Number of attention mechanisms
    """
    del (
        embedding_size,
        num_unrestricted_heads,
    )
    attns = [
        MultiHeadAttention(
    ...
    def f(
        *,
        ...
        def attn_fn(attn, inputs, **kwargs):
            attn_vec, attn_weights, value_heads = attn(inputs=inputs, **kwargs)
            return attn_vec, {
                "attention_weights": attn_weights,
                "value_heads": value_heads,
            }

        h_attention, attention_weights = switch_with_aux(
            attns,
            attn_fn,
            attn_indicator,
            inputs=inputs,
...
class Core(hk.Module):
    ...
    def __call__(
        ...
        jnp.ndarray,
        Dict[str, Dict[int, Dict[str, jnp.ndarray]]],
    ]:
        """Computes the logits, next memory, layer outputs and attention weights.
        ...
        Returns:
            A tuple containing:
                - The final layer embeddings of shape [B, T, D]
                - The new memory if `memory` is given else the constant 0
                - Layer outputs of shape [B, T, L, D]
                - Attention weights dictionary with structure:
                  {
                    'restricted': {
                        layer_idx: {
                            'attention_weights': weights,
                  ...
                  where weights have shape [B, H, T, T+M] and
                  values have shape [B, T+M, H, V]
        """
        ...
        if self._num_unrestricted_heads is not None:
            # HACK: We don't use this feature, so we don't support it.
            ...
            block_kwargs = dict(num_unrestricted_heads=self._num_heads)
        ...
        layers_outputs = [h]
        attention_weights: Dict[str, Dict[int, Dict[str, jnp.ndarray]]] = {
            "restricted": {},
            "unrestricted": {},
        }

        for i, (block, unrestricted_layer) in enumerate(blocks):
            # Memory handling code remains the same
            if new_memory is not None:
                if not use_smart_memory:
                    new_mem = jnp.concatenate((memory[i], h), axis=1)
                    new_mem = lax.slice(
                        new_mem,
                        start_indices=(0, new_mem.shape[1] - self._memory_length, 0),
            ...
            # Call the block and collect attention weights
            h, layer_attention_weights = block(
                is_training=is_training,
                inputs=h,
                attention_mask=attention_mask,
            ...
            # Store attention weights by layer and type
            if unrestricted_layer:
                attention_weights["unrestricted"][i] = layer_attention_weights
            else:
                attention_weights["restricted"][i] = layer_attention_weights
        ...
        return (
            h,  # [B, T, H]
            new_memory,  # [N_layers, B, M, H] or None
