...
from transformer_grammars.data import preprocessing, sp_utils, text_dataset
...
def _loss(apply, maskrules, vocab_size, params, state, rng, batch):
    ...
    (logits, _, _), state = apply(params, state, rng=rng, **inputs)
...
def _build_update(config, maskrules, token_type_ranges):
    ...
    def update(training_state, batch):
        ...
        if clip_grad_norm:
            ...
            clipped_grads = jax.tree_util.tree_map(lambda t: t * clipping_factor, grads)
        else:
            clipped_grads = grads
        clipped_grad_norm = optax.global_norm(clipped_grads)

        # Compute and apply updates via our optimizer.
        learning_rate = _learning_rate(config.training.lr_schedule, training_state.step)
...
def _build_evaluator(eval_cfg, model_cfg, maskrules, token_type_ranges):
    ...
    def eval_epoch(py_step, training_state):
        ...
        return {"total_loss": total_loss, "total_count": total_count}
...
def _log(unused_cfg, py_step, metrics):
    metrics_str = "\t".join((f"{k}={_get_first(v)!s}" for (k, v) in metrics.items()))
    logging.info("[train % 9d] %s", py_step, metrics_str)


def _save_checkpoint(unused_cfg, py_step, training_state, model_cfg, save_path):
...
def _reload_from_checkpoint(_, current_state, save_path):
    ckpt = checkpoint.load_checkpoint(save_path)
...
def main(config, _):
    ...
    if logging_dir:
        logging.get_absl_handler().use_absl_log_file("train.log", logging_dir)

    seed = config.get("seed", 0)
    logging.info("Using seed %d.", seed)

    save_path = config.get("save_path", "checkpoint.pkl")
    logging.info("Saving checkpoints to %s.", save_path)
    ...
    init_rng, train_rng = jax.pmap(
        functools.partial(_split_init_and_train_rngs, seed), axis_name="i"
    )(
    ...
    try:
        training_state, py_step = _reload_from_checkpoint(
            None, training_state, save_path
        )
    except checkpoint.CheckpointLoadingError:
        logging.warning("No checkpoint found, or unusable -- starting from scratch.")
    else:
        logging.warning("Checkpoint found -- restarting from step %d.", py_step)

    # Training loop.
    logging.info("Starting training.")
    saved_eval_loss = float("inf")
    while py_step < config.training.num_steps:
        ...
        if last or _should_do(config.checkpointing, py_step):
            eval_info = evaluator(py_step, training_state)
            current_eval_loss = eval_info["total_loss"] / eval_info["total_count"]

            if current_eval_loss < saved_eval_loss:
                _save_checkpoint(
                    config.checkpointing, py_step, training_state, config.model, save_path
                )
                saved_eval_loss = current_eval_loss
