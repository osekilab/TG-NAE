...
# extract the terminals from the trees
python ./src/syntactic_attention_based_metric_transformer_grammars/extract_terminals.py \
    --train-input ${DATA}/train.txt \
    --valid-input ${DATA}/valid.txt \
    --test-input ${DATA}/test.txt \
    --train-output ${DATA}/train.terminals.txt \
    --valid-output ${DATA}/valid.terminals.txt \
    --test-output ${DATA}/test.terminals.txt
...
# do
#   python ./src/syntactic_attention_based_metric_transformer_grammars/tools/convert_to_choe_charniak.py --input ${DATA}/${SPLIT}.txt --output ${DATA}/${SPLIT}.choecharniak
# done
...
TOTAL_VOCAB_SIZE=32768

SPM=./data/bllip-lg/spm
mkdir -p ${SPM}
...
spm_encode \
  --output_format=id \
  --model=${SPM}/spm.model \
  --input=${DATA}/${SPLIT}.terminals.txt \
  --output=${DATA}/${SPLIT}.terminals.enc
done
...
do
  ...
  --input ${DATA}/${SPLIT}.terminals.enc \
  --output ${DATA}/${SPLIT}.terminals.csv \
  --vocab ${SPM}/spm.vocab
done
